...
function CourseCard({ course }: { course: CourseFromAPI }) {
    ...
    return (
        <div className={styles.courseCard}> {/* ตรวจสอบว่ามี style นี้ใน course.module.css */}
            <div className={styles.imageWrapper}> {/* ตรวจสอบ style นี้ */}
                <Image
                    src={imageUrl}
                    alt={course.courseName}
                    width={180} height={240} // หรือใช้ fill ถ้า container มีขนาด
                    style={{ objectFit: 'cover' }}
                    onError={(e) => { (e.target as HTMLImageElement).src = defaultCourseImage.src; }}
                />
            </div>
            <p className={styles.price}>{course.price.toLocaleString()} บาท</p> {/* ตรวจสอบ style นี้ */}
            <p className={styles.courseTitle}>{course.courseName}</p> {/* ตรวจสอบ style นี้ */}
            <p className={styles.code}>รหัส {course.courseNumber}</p> {/* ตรวจสอบ style นี้ */}
            <div className={styles.actions}> {/* ตรวจสอบ style นี้ */}
                <span className={styles.cart}>🛒</span>
                <button className={styles.detailBtn}>รายละเอียด</button> {/* ตรวจสอบ style นี้ */}
            </div>
        </div>
    );
}
...
export default function CoursePage() {
    ...
    const filteredCourses = useMemo(() => {
        console.log("Filtering courses. Levels:", selectedLevels, "Types:", selectedTypes); // Debug log
        return allDbCourses.filter((course) => {
            ...
            const levelMatch = selectedLevels.length === 0 || (course.level && selectedLevels.includes(course.level));

            // ตรวจสอบ Type (ใช้ course.category จาก API)
             // ทำให้เป็น Case-insensitive หรือ Normalization ถ้าจำเป็น
            const typeMatch = selectedTypes.length === 0 || (course.category && selectedTypes.includes(course.category));
    ...
    return (
        ...
        <main className={styles.pageWrapper}> {/* ตรวจสอบ style นี้ */}
            {/* Sidebar สำหรับ Filter */}
            <aside className={styles.sidebar}> {/* ตรวจสอบ style นี้ */}
            ...
            <section className={styles.mainContent}> {/* ตรวจสอบ style นี้ */}
                <h2 className={styles.pageTitle}>คอร์สเรียน</h2> {/* ตรวจสอบ style นี้ */}

                {isLoading && <p className="text-center py-5">Loading courses...</p>}
                {error && <p className="text-center py-5 text-red-600">Error: {error}</p>}

                {!isLoading && !error && (
                     <div className={styles.courseList}> {/* ตรวจสอบ style นี้ */}
                         {filteredCourses.length > 0 ? (
                            ...
                            filteredCourses.map((course) => (
                             ...
                             <p className="col-span-full text-center text-gray-500 py-10">ไม่พบคอร์สเรียนตามเงื่อนไขที่เลือก</p> // เพิ่มข้อความเมื่อไม่เจอ
                         )}
                    </div>
                 )}
            </section>
