...
type ScheduledSessionFromAPI = { // ควรตรงกับที่ API ส่งกลับมา
    id: number;
    courseId: number;
    title: string | null;
...
export default function AdminCalendarPage() {
    ...
    const [formData, setFormData] = useState<SessionFormData>({
        courseId: '', title: '', description: '', startTime: '', endTime: '', location: ''
    });
    ...
    const fetchData = async (fetchInfo?: { startStr?: string, endStr?: string }) => {
        setIsLoading(true); setPageError(null);
        let coursesData: CourseInfo[] = courses; // Keep existing courses if already fetched
        if(courses.length === 0){ // Fetch courses only once or if needed
            try {
                ...
                coursesData = Array.isArray(coursesJson) ? coursesJson : coursesJson.courses || [];
                setCourses(coursesData);
                if(coursesData.length > 0 && formData.courseId === '') { // Set default courseId for new events
                    setFormData(prev => ({...prev, courseId: coursesData[0].id.toString()}));
                }
            } catch (err) {
                 console.error("Error fetching courses:", err);
                 setPageError(err instanceof Error ? err.message : "Failed to load courses");
        ...
        try {
            ...
            if (!sessionsRes.ok) throw new Error('Failed to fetch scheduled sessions');
            ...
            setPageError(prev => prev ? `${prev}; ${err instanceof Error ? err.message : "Failed to load sessions"}` : (err instanceof Error ? err.message : "Failed to load sessions"));
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDateClick = (arg: DateClickArg) => {
        ...
        setFormData({
            ...
            endTime: formatDateTimeLocal(new Date(arg.date.getTime() + 60 * 60 * 1000)), // Default 1 hour duration
            location: ''
        });
        setIsModalOpen(true);
    };
    ...
    const handleEventClick = (clickInfo: EventClickArg) => {
        ...
        setFormData({
            ...
            endTime: formatDateTimeLocal(clickInfo.event.end || clickInfo.event.start),
            location: props.location || ''
        });
        setIsModalOpen(true);
    };

    const handleEventChange = async (changeInfo: EventDropArg | FullCalendarEventResizeDoneArg) => {
         if (!window.confirm(`คุณต้องการย้าย/ปรับขนาด "${changeInfo.event.title}" ใช่หรือไม่?`)) {
             changeInfo.revert();
             return;
         }
         try {
             const payload = {
                 ...
                 endTime: changeInfo.event.end?.toISOString() || changeInfo.event.start?.toISOString(),
                 // ส่งเฉพาะ field ที่เปลี่ยนก็ได้ แต่ startTime/endTime น่าจะเปลี่ยนเสมอ
             };
             const response = await fetch(`/api/scheduled-sessions/${changeInfo.event.id}`, {
                 method: 'PUT',
    ...
    const handleModalSubmit = async (e: FormEvent) => {
        ...
        if (!formData.courseId || !formData.startTime || !formData.endTime) {
            setFormError("กรุณาเลือกคอร์ส และระบุวันเวลาเริ่มต้น-สิ้นสุด"); return;
        }
        const start = new Date(formData.startTime);
        const end = new Date(formData.endTime);
        if (end <= start) {
            setFormError("เวลาสิ้นสุดต้องอยู่หลังเวลาเริ่มต้น"); return;
        }
        ...
        try {
            ...
            let response;
            if (modalMode === 'add') {
                response = await fetch('/api/scheduled-sessions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
            } else if (selectedEventId) {
                response = await fetch(`/api/scheduled-sessions/${selectedEventId}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
            ...
            if (calendarApi) {
                ...
                calendarApi.addEvent({
                    ...
                    title: `${savedSession.course.courseName}${savedSession.title ? ` - ${savedSession.title}` : ''}`,
                    ...
                    extendedProps: {
                        ...
                        originalTitle: savedSession.title
                    }
                });
            }
    ...
    const handleDeleteEvent = async () => {
        if (!selectedEventId || !window.confirm("คุณต้องการลบตารางสอนนี้จริงหรือไม่?")) return;
        setIsSubmitting(true); setFormError(null);
        try {
            const response = await fetch(`/api/scheduled-sessions/${selectedEventId}`, { method: 'DELETE' });
    ...
    if (isLoading && events.length === 0 && courses.length === 0) { // Initial full load
        return <div className="p-6 text-center">Loading calendar and initial data...</div>;
    }
    if (pageError && events.length === 0 && courses.length === 0) { // Critical error on initial load
        return <div className="p-6 text-center text-red-500">Error: {pageError}</div>;
    }

    return (
        <div className="p-4 sm:p-6 lg:p-8"> {/* ควรถูกครอบด้วย AdminLayout */}
            <div className="sm:flex sm:items-center sm:justify-between mb-6">
                 <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">ปฏิทินการเรียนการสอน</h1>
                 <button
                    type="button"
                    onClick={() => handleDateClick({ date: new Date(), dateStr: new Date().toISOString().split('T')[0], allDay: false } as DateClickArg)} // เปิด Modal แบบ Add
            ...
            <div className="bg-white p-2 sm:p-4 rounded-lg shadow"> {/* ปรับ Padding ของ Calendar Wrapper */}
                <FullCalendar
                    ref={calendarRef}
                    ...
                    datesSet={(dateInfo) => {
                        console.log("Calendar datesSet:", dateInfo.startStr, dateInfo.endStr);
                ...
                {/* ... (JSX ของ Modal เหมือนตัวอย่างก่อนหน้า ที่มี Form, Inputs, Buttons) ... */}
                 ...
                 <div className="fixed inset-0 flex min-h-full items-center justify-center p-4">
                     <Dialog.Panel className="w-full max-w-md rounded-lg bg-white p-6 shadow-xl max-h-[90vh] flex flex-col">
                        ...
                        <form onSubmit={handleModalSubmit} className="space-y-4 overflow-y-auto flex-grow">
                            {formError && <p className="text-sm text-red-600 bg-red-50 p-2 rounded">{formError}</p>}
                            <div>
                                <label htmlFor="courseId" className="block text-sm font-medium text-gray-700">คอร์สเรียน</label>
                                 <select id="courseId" name="courseId" required value={formData.courseId} onChange={(e)=>setFormData({...formData, courseId: e.target.value})} disabled={isSubmitting}
                                    ...
                                    {courses.map(course => (<option key={course.id} value={course.id.toString()}>{course.courseName}</option>))}
                                </select>
                            </div>
                            <div>
                                <label htmlFor="title" className="block text-sm font-medium text-gray-700">หัวข้อเฉพาะ (ถ้ามี)</label>
                                <input type="text" id="title" name="title" value={formData.title} onChange={(e)=>setFormData({...formData, title: e.target.value})} disabled={isSubmitting}
                                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm ..."/>
                            </div>
                             <div>
                                <label htmlFor="startTime" className="block text-sm font-medium text-gray-700">เวลาเริ่มต้น</label>
                                <input type="datetime-local" id="startTime" name="startTime" required value={formData.startTime} onChange={(e)=>setFormData({...formData, startTime: e.target.value})} disabled={isSubmitting}
                                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm ..."/>
                            </div>
                            <div>
                                <label htmlFor="endTime" className="block text-sm font-medium text-gray-700">เวลาสิ้นสุด</label>
                                <input type="datetime-local" id="endTime" name="endTime" required value={formData.endTime} onChange={(e)=>setFormData({...formData, endTime: e.target.value})} disabled={isSubmitting}
                                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm ..."/>
                            </div>
                             <div>
                                <label htmlFor="location" className="block text-sm font-medium text-gray-700">สถานที่/Link (ถ้ามี)</label>
                                <input type="text" id="location" name="location" value={formData.location} onChange={(e)=>setFormData({...formData, location: e.target.value})} disabled={isSubmitting}
                                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm ..."/>
                            </div>
                             <div>
                                <label htmlFor="description" className="block text-sm font-medium text-gray-700">รายละเอียด (ถ้ามี)</label>
                                <textarea id="description" name="description" rows={3} value={formData.description} onChange={(e)=>setFormData({...formData, description: e.target.value})} disabled={isSubmitting}
                                    className="mt-1 block w-full rounded-md border-gray-300 shadow-sm ..."/>
                            </div>
                        </form>
                        <div className="mt-6 flex justify-end gap-3 border-t pt-4">
                             {modalMode === 'edit' && selectedEventId && ( // แสดงปุ่มลบเฉพาะตอน Edit และมี Event ID
                                <button type="button" onClick={handleDeleteEvent} disabled={isSubmitting}
                            ...
                            <button type="submit" form="achievementForm" /* ควรเปลี่ยน form id เป็น sessionForm */ onClick={handleModalSubmit} disabled={isSubmitting}
                                className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-md hover:bg-blue-700 disabled:opacity-50">
                                {isSubmitting ? 'กำลังบันทึก...' : (modalMode === 'add' ? 'เพิ่มตาราง' : 'บันทึกการแก้ไข')}
                            </button>
                        </div>
                     </Dialog.Panel>
                 </div>
